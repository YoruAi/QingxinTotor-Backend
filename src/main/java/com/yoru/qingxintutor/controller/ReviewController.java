...
public class ReviewController {
    ...
    @RequireStudent
    @PostMapping
    public ApiResult<TeacherReviewEntity> createReview(@AuthenticationPrincipal CustomUserDetails userDetails,
                                                       @Valid @RequestBody ReviewCreateRequest request) {
        TeacherReviewEntity result = reviewService.create(userDetails.getUser().getId(), request);
        return ApiResult.success(result);
    }

    @RequireStudent
    @PutMapping("/{id}")
    public ApiResult<TeacherReviewEntity> updateReview(@AuthenticationPrincipal CustomUserDetails userDetails,
                                                       @PathVariable("id")
                                                       @Min(value = 1, message = "Id must be a positive number")
                                                       Long id,
                                                       @Valid @RequestBody ReviewUpdateRequest request) {
        TeacherReviewEntity result = reviewService.update(userDetails.getUser().getId(), id, request);
        return ApiResult.success(result);
    }
